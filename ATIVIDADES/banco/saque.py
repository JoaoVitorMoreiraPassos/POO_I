...
class Valor():
    def __init__(self,banco, cod):
        ...
        self.btncon = Button(self.main, text="Confirmar",anchor="center", command=partial(self.saque, banco, cod))   
    ...
    def saque(self, banco, cod):
        # Extrai os valores dos inputs, nome e senha
        valor = self.ent.get()
        senha = self.ent_senha.get()
        # Tenta fazer o saque com os valores extraídos
        resultado, mensagem = banco.sacarValor(cod,valor, senha)
        if(resultado):# Se o saque for finalizado com sucesso
        ...
        else:# Se ocorrer algum erro  no saque, informa o erro ao usuário.
            self.aviso['text'] = mensagem
    ...
    def __init__(self, banco):
        ...
        if(resultado):# Se a busca ocorrer bem
            ...
            for i in range(0, len(dados)-2, 3):
                ...
                # chamam o método saque que irá abir um menu para confirmar os valores a serem sacados
                if(i>0):
                    btns.append(Button(self.second_frame, text="Selecionar", command=partial(self.saque, banco,dados[i].strip()), bg="#70aa87", cursor="hand2", anchor="center"))
